...
describe('Comentario CRUD tests', function () {
  ...
  it('should be able to save a Comentario if logged in', function (done) {
  ...
  it('should not be able to save an Comentario if no name is provided', function (done) {
  ...
  it('should be able to get a single Comentario if not signed in', function (done) {
    ...
    comentarioObj.save(function () {
      request(app).get('/api/comentarios/' + comentarioObj._id)
        .end(function (req, res) {
          // Set assertion
          res.body.should.be.instanceof(Object).and.have.property('name', comentario.name);
  ...
  it('should be able to get a single Comentario that has an orphaned user reference', function (done) {
        ...
        .end(function (signinErr, signinRes) {
          ...
          agent.post('/api/comentarios')
            ...
            .end(function (comentarioSaveErr, comentarioSaveRes) {
              ...
              // Set assertions on new Comentario
              (comentarioSaveRes.body.name).should.equal(comentario.name);
              should.exist(comentarioSaveRes.body.user);
              should.equal(comentarioSaveRes.body.user._id, orphanId);

              // force the Comentario to have an orphaned user reference
              orphan.remove(function () {
                ...
                agent.post('/api/auth/signin')
                  ...
                  .end(function (err, res) {
                    ...
                    // Get the Comentario
                    agent.get('/api/comentarios/' + comentarioSaveRes.body._id)
                      .expect(200)
                      .end(function (comentarioInfoErr, comentarioInfoRes) {
                        // Handle Comentario error
                        if (comentarioInfoErr) {
                          return done(comentarioInfoErr);
                        }

                        // Set assertions
                        (comentarioInfoRes.body._id).should.equal(comentarioSaveRes.body._id);
                        (comentarioInfoRes.body.name).should.equal(comentario.name);
                        should.equal(comentarioInfoRes.body.user, undefined);
